...
void Autotalent::init(Patch* patch)
{
    unsigned long ti;
    unsigned long SampleRate;
    
    SampleRate = patch->getSampleRate();
    ...
    mcbi = patch->createMemoryBuffer(1, mcbsize)->getSamples(0);    //input buffer
    mcbf = patch->createMemoryBuffer(1, mcbsize)->getSamples(0);    //formant correction buffer
    mcbo = patch->createMemoryBuffer(1, mcbsize)->getSamples(0);    //output buffer
    mhannwindow = patch->createMemoryBuffer(1, mcbsize)->getSamples(0);
    mcbwindow = patch->createMemoryBuffer(1, mcbsize)->getSamples(0);
    mffttime = patch->createMemoryBuffer(1, mcbsize)->getSamples(0);
    mfftfreqre = patch->createMemoryBuffer(1, mcorrsize)->getSamples(0);
    mfftfreqim = patch->createMemoryBuffer(1, mcorrsize)->getSamples(0);
    macwinv = patch->createMemoryBuffer(1, mcbsize)->getSamples(0);
    mfrag = patch->createMemoryBuffer(1, mcbsize)->getSamples(0);
    
    
//    mcbi = (float*) calloc(mcbsize, sizeof(float));   //replaced with assignment to MemoryBuffer
...
//    mhannwindow = (float*) calloc(mcbsize, sizeof(float)); //replaced with assignment to MemoryBuffer
    for (ti=0; ti<mcbsize; ti++) {
        mhannwindow[ti] = -0.5*cos(2*PI*ti/mcbsize) + 0.5;
    }
    
    // Generate a window with a single raised cosine from N/4 to 3N/4
//    mcbwindow = (float*) calloc(mcbsize, sizeof(float));  //replaced with assignment to MemoryBuffer
...
//    mfmembvars = fft_con(mcbsize);
    fft_init(&mfmembvars, mcbsize, patch->createMemoryBuffer(1, mcbsize)->getSamples(0));
    
//    mffttime =   (float*) calloc(mcbsize, sizeof(float));     //replaced with assignment to MemoryBuffer
...
//    macwinv = (float*) calloc(mcbsize, sizeof(float));    //replaced with assignment to MemoryBuffer
    for (ti=0; ti<mcbsize; ti++) {
    ...
    mphasein = 0;
    mphaseout = 0;
//    mfrag = (float*) calloc(mcbsize, sizeof(float));  //replaced with assignment to MemoryBuffer
    mfragsize = 0;
