...
/*
 ...
 Modified by Guillaume Le Nost, 2016.
...
#ifndef __LpfDelay2Patch_hpp__
#define __LpfDelay2Patch_hpp__
...
class LpfDelay2Patch : public Patch {
  static const int REQUEST_BUFFER_SIZE = 1<<16;
private:
  CircularBuffer* delayBuffer;
  int delay;
  float alpha, dryWet;
  LpfDelay2::ToneBiquad filter;
  StereoBiquadFilter* highpass;
public:
  LpfDelay2Patch() : delay(0), alpha(0.04), dryWet(0.f){
  ...
  void processAudio(AudioBuffer &buffer){
    float delayTime, feedback, fc, dly, gate;
    ...
    for(int n = 0; n < size; n++){
      ...
      delayBuffer->write(feedback * dly + gate * x[n]);  //filter.processSample(x[n], size, n));
