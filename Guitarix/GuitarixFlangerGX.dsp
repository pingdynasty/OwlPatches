...
process = h_flanger(dmax, curdel,  fb, wet1) :
    ...
    freq = hslider("Freq[log][name:Speed][unit:Hz][style:knob][OWL:PARAMETER_C]", 0.2, 0.05, 10, 0.01);
    fb = hslider("Feedback[name:Feedback][style:knob][OWL:PARAMETER_A]", -0.707, -0.99, 0.99, 0.01) * (2+abs(mix))/3;
    lfo = component("oscillator.lib").oscrs;
    curdel = odflange+dflange*(1+lfo(freq))/2;
    //curdel = hslider("Delay", 20, 0, 400, 0.1);
    wet = hslider("Wet[name:Wet][style:knob][OWL:PARAMETER_D]", 100, 0, 100, 1)/100;
